...
const ERROR_STATUS: Record<RunnerErrorCode, number> = {
  ...
  // Livraison Git. Un `409` pour tout ce qui a change depuis la validation :
  // la requete est valide, l'etat du repository ne l'est plus. Un `422` pour
  // ce qui manque a la machine, un `500` pour ce que Git n'a pas su faire.
  [RUNNER_ERROR.DELIVERY_REPOSITORY_CHANGED]: 409,
  [RUNNER_ERROR.DELIVERY_INDEX_NOT_EMPTY]: 409,
  [RUNNER_ERROR.DELIVERY_IDENTITY_MISSING]: 422,
  [RUNNER_ERROR.DELIVERY_STAGING_FAILED]: 500,
  [RUNNER_ERROR.DELIVERY_STAGED_MISMATCH]: 409,
  [RUNNER_ERROR.DELIVERY_COMMIT_FAILED]: 500,
  [RUNNER_ERROR.DELIVERY_TREE_MISMATCH]: 409,
  [RUNNER_ERROR.DELIVERY_PUSH_REJECTED]: 409,
  [RUNNER_ERROR.DELIVERY_PUSH_FAILED]: 502,
